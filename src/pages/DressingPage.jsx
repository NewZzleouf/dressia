    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto animate-fadeIn">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-ink-400 text-sm mt-1">{wardrobe.length} pièce(wardrobe.length !== 1 ? 's' : '') · {isPremium ? 'Sauvegardé ✦' : 'Session uniquement'}</p>
        </div>
        <button onClick={() => fileRef.current?.click()} disabled={analyzing} className="flex items-center gap-2 bg-ink-900 text-cream-100 font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-ink-700 disabled:opacity-50">
          ...
          {analyzing ? 'Analyse…' : 'Ajouter une pièce'}
        </button>
      ...
      {!isPremium && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-2"><Lock size={14} className="text-amber-500" /><p className="text-sm text-amber-700">Version gratuite — dressing non sauvegardé</p></div>
      ...
      {pendingItem && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="font-display text-2xl italic text-ink-800 mb-5">Vêtement détecté</h3>
      ...
      {wardrobe.length === 0 ? (
        <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-cream-300 rounded-3xl p-16 text-center cursor-pointer hover:border-cream-400">
          ...
          <p className="text-ink-400 text-sm">Clique pour ajouter ta première pièce</p>
        </div>
      ) : filtered.length === 0 ? (
        <p className="text-center text-ink-400 py-12">Aucune pièce dans cette catégorie.</p>
      ) : (
          ...
          {filtered.map(item => (
            <div key={item.id} className="group bg-white border border-cream-200 rounded-2xl overflow-hidden hover:shadow-md">
              <div className="relative aspect-[3/4] bg-cream-50">
                {item.image_url ? <img src={item.image_url} alt={item.nom} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-4xl">{CAT_ICONS[item.categorie] || '👕'}</div>}
              ...
              <div className="p-3"><p className="text-sm font-medium text-ink-800 truncate">{item.nom}</p><p className="text-xs text-ink-400 mt-0.5">{item.couleur} · {item.style}</p></div>
            </div>
          ))}
        </div>
      )}
